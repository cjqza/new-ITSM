(function () {
  ...
  let ITSM_SESSION = readSession();
  function readSession() {
    try { return JSON.parse(localStorage.getItem('itsm.session') || 'null'); } catch (error) { return null; }
  }
  function saveSession(value) {
    try { localStorage.setItem('itsm.session', JSON.stringify(value)); } catch (error) {}
  }
  function redirectToPortalIfStandalone() {
    if (window.self === window.top && !window.location.search.includes('direct=1')) {
  ...
  function hasItsmAccess() {
    const roles = (ITSM_SESSION && ITSM_SESSION.roles) || [];
    ...
    appRoot.innerHTML = `
      <div class="login-screen">
        <section class="login-card">
          <div class="login-brand"><span class="brand-mark">ITSM</span><div><strong>客服工单系统</strong><small>权限不足</small></div></div>
          <div class="login-heading"><span class="eyebrow">无访问权限</span><h1>您还没有 ITSM 系统权限</h1><p>请先到用户门户申请 ITSM 权限，管理员审批通过后重新登录即可使用。</p></div>
          <div class="login-note"><a href="./user-portal.html">返回用户门户</a></div>
        </section>
      </div>
    `;
  }
  ...
  async function apiGet(path) {
    ...
    if (!response.ok || body.code !== 'SUCCESS') throw new Error(body.message || `HTTP ${response.status}`);
    return body.data;
  }
  async function apiPost(path, payload) {
    const response = await fetch(path, { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok || body.code !== 'SUCCESS') throw new Error(body.message || `HTTP ${response.status}`);
    return body.data;
  }
  function roleToProfileKey(roles) {
    const list = roles || [];
    if (list.includes('SUPPORT_ADMIN')) return 'admin';
    if (list.includes('SUPPORT_AGENT')) return 'support';
  ...
  function mapSupportTicket(item) {
    return {
      ...
      requester: item.requester ? { userId: item.requester.userId, displayName: item.requester.displayName || '—', departmentName: item.requester.departmentName || '' } : { userId: null, displayName: '—', departmentName: '' },
      title: item.title,
      description: '',
      source: 'MANUAL',
      status: item.status,
      ...
      queueId: null,
      classification: null,
      assignee: item.assignee ? { userId: item.assignee.userId, displayName: item.assignee.displayName || item.assignee.userId } : null,
      conversation: null,
      statusHistory: [],
      auditEvents: [],
      resolution: null,
  ...
  async function loadRealTickets() {
    try {
      if (isSupportRole() || isSupervisorRole()) {
        const [pending, progress, confirm, history] = await Promise.all([
          apiGet('/api/v1/support/tickets?view=PENDING&page=1&pageSize=100'),
          apiGet('/api/v1/support/tickets?view=IN_PROGRESS&page=1&pageSize=100'),
          apiGet('/api/v1/support/tickets?view=PENDING_CONFIRM&page=1&pageSize=100'),
          apiGet('/api/v1/support/tickets?view=HISTORY&page=1&pageSize=100')
        ]);
        const merged = [].concat(pending.items || [], progress.items || [], confirm.items || [], history.items || []);
        DATA.tickets = merged.map(mapSupportTicket);
      } else {
        const page = await apiGet('/api/v1/tickets?page=1&pageSize=100');
        DATA.tickets = (page.items || []).map(mapUserTicket);
      }
    } catch (error) {
      DATA.tickets = [];
    }
  ...
  function apiDictType(tabKey) {
    return tabKey === 'BUSINESS_SYSTEM' ? 'BUSINESS_LINE' : tabKey;
  }
  ...
  async function loadRealDictTab(tabKey) {
    const type = apiDictType(tabKey);
    try {
      const page = await apiGet('/api/v1/admin/dictionaries/' + type + '/items?enabledOnly=false&page=1&pageSize=200');
      DATA.dictionaries[tabKey] = {
        dictType: tabKey,
        title: DICT_TITLES[type] || type,
        description: '',
        items: (page.items || []).map(mapDictItem)
      };
    } catch (error) {
      // 保留当前数据，避免整页空白。
    }
  ...
  async function loadRealDictionaries() {
    const tabKeys = ['BUSINESS_SYSTEM', 'MANAGEMENT_UNIT', 'SYMPTOM', 'REASON', 'SOLUTION_METHOD'];
    await Promise.all(tabKeys.map((tabKey) => loadRealDictTab(tabKey)));
  }

  function currentDictItems() {
    const dict = DATA.dictionaries[state.activeDictTab];
    return dict ? (dict.items || []) : [];
  }

  async function createDictItemReal() {
    ...
    if (!code || !name) {
      showToast('保存失败', 'code/name 为必填项', 'error');
      return;
    }
    try {
      await apiPost('/api/v1/admin/dictionaries/' + apiDictType(state.activeDictTab) + '/items', { code, name, parentId, sort });
      state.overlay = null;
      await loadRealDictTab(state.activeDictTab);
      renderStart();
      renderOverlayWrapper();
      showToast('保存成功', '字典项已新增', 'success');
    } catch (error) {
      showToast('保存失败', error.message, 'error');
    }
  }

  async function updateDictItemReal(itemId) {
    const form = document.getElementById('dict-form');
    if (!form) return;
    const values = new FormData(form);
    const name = String(values.get('name') || '').trim();
    ...
    const item = currentDictItems().find((entry) => entry.itemId === itemId);
    if (!item) {
      showToast('保存失败', 'RESOURCE_NOT_FOUND', 'error');
      return;
    }
    try {
      await apiPatch('/api/v1/admin/dictionaries/items/' + encodeURIComponent(itemId), { name, parentId, sort, version: item.version, description });
      state.overlay = null;
      await loadRealDictTab(state.activeDictTab);
      renderStart();
      renderOverlayWrapper();
      showToast('保存成功', '字典项已更新', 'success');
    } catch (error) {
      showToast('保存失败', error.message, 'error');
    }
  }

  async function disableDictItemReal(itemId) {
    const form = document.getElementById('disable-form');
    if (!form) return;
    const values = new FormData(form);
    const reason = String(values.get('reason') || '').trim();
    if (!reason) {
      showToast('保存失败', 'reason 不能为空', 'error');
      return;
    }
    const item = currentDictItems().find((entry) => entry.itemId === itemId);
    if (!item) {
    ...
    try {
      await apiPost('/api/v1/admin/dictionaries/items/' + encodeURIComponent(itemId) + '/disable', { reason, version: item.version });
      state.overlay = null;
      await loadRealDictTab(state.activeDictTab);
      renderStart();
      renderOverlayWrapper();
  ...
  function mapUserTicket(item) {
    return {
      ticketId: item.ticketId,
      ticketNo: item.ticketNo,
      tenantId: currentTenant().tenantId,
      requester: { userId: currentUser().userId, displayName: currentUser().displayName, departmentName: currentUser().departmentName },
      title: item.title,
      description: '',
      source: 'MANUAL',
      status: item.status,
  ...
  async function newSessionReal() {
    try {
      const created = await apiPost('/api/v1/conversations/sessions', { channel: 'WORKBENCH', subject: '新会话' });
  ...
  async function sendChatMessageReal(text) {
    try {
      let sessionId = getCurrentSession() ? getCurrentSession().sessionId : null;
      if (!sessionId) {
        const created = await apiPost('/api/v1/conversations/sessions', { channel: 'WORKBENCH', subject: text.slice(0, 40) });
        sessionId = created.sessionId;
      }
      await apiPost('/api/v1/conversations/sessions/' + encodeURIComponent(sessionId) + '/messages', {
        clientMessageId: 'cli_' + Date.now() + '_' + Math.random().toString(36).slice(2, 10),
        content: text
      });
      await loadRealSessions();
  ...
  async function handoffReal(sessionId) {
    try {
      await apiPost('/api/v1/conversations/sessions/' + encodeURIComponent(sessionId) + '/handoff', {
        reason: '用户请求转人工',
      ...
      renderStart();
      renderOverlayWrapper();
      showToast('已转人工', '已记录转人工请求', 'success');
    } catch (error) {
      showToast('操作失败', error.message, 'error');
    }
  }

  async function createTicketFromSessionReal() {
    const session = getCurrentSession();
    if (!session) {
      showToast('操作失败', '请先创建会话', 'error');
      return;
    }
    try {
      ...
      session.ticketId = created.ticketId;
      await loadRealTickets();
      state.activeTicketId = created.ticketId;
      renderStart();
      renderOverlayWrapper();
      showToast('已创建工单', `工单号 ${created.ticketNo}`, 'success');
    } catch (error) {
      showToast('操作失败', error.message, 'error');
    }
  }
  async function doRealLogin(tenantId, account, password) {
    ...
    ITSM_SESSION = { accessToken: data.accessToken, tenantId, user: data.user, tenant: data.tenant, roles: data.roles || [] };
    saveSession(ITSM_SESSION);
    if (!hasItsmAccess()) {
      window.location.replace('user-portal.html');
      return;
    }
    if (redirectToPortalIfStandalone()) return;
    state.activeProfileKey = roleToProfileKey(ITSM_SESSION.roles);
    await Promise.all([loadRealTickets(), loadRealDictionaries(), loadRealSessions()]);
    renderStart();
    renderOverlayWrapper();
  ...
  async function closeTicketReal(ticketId) {
    try {
      await apiPost('/api/v1/support/tickets/' + encodeURIComponent(ticketId) + '/close', { closeReason: '联调关闭', note: null });
      await loadRealTickets();
      state.overlay = null;
      renderStart();
      renderOverlayWrapper();
      showToast('已关闭', '工单已关闭', 'success');
    } catch (error) {
      showToast('操作失败', error.message, 'error');
    }
  }
  ...
  async function apiPatch(path, payload) {
    const response = await fetch(path, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify(payload) });
    const body = await response.json().catch(() => ({}));
    if (!response.ok || body.code !== 'SUCCESS') throw new Error(body.message || `HTTP ${response.status}`);
    return body.data;
  }

  function mapTicketDetailToTicket(detail) {
    return {
      ticketId: detail.ticketId,
      ticketNo: detail.ticketNo,
      tenantId: detail.tenantId,
      requester: { userId: detail.requester ? detail.requester.userId : null, displayName: detail.requester && detail.requester.displayName ? detail.requester.displayName : '—', departmentName: detail.requester && detail.requester.departmentName ? detail.requester.departmentName : '' },
      title: detail.title,
      ...
      queueId: null,
      classification: detail.classification || { managementUnitId: null, symptomId: null, reasonId: null, solutionMethodId: null, customReason: null, customSolution: null },
      assignee: detail.assignee ? { userId: detail.assignee.userId, displayName: detail.assignee.displayName || detail.assignee.userId } : null,
      conversation: detail.conversation ? { sessionId: detail.conversation.sessionId || null, summary: detail.conversation.summary || '', messageCount: typeof detail.conversation.messageCount === 'number' ? detail.conversation.messageCount : 0 } : { sessionId: null, summary: '', messageCount: 0 },
      statusHistory: (detail.statusHistory || []).map((h) => ({ status: h.status, occurredAt: h.occurredAt, operator: h.operator, note: h.note })),
  ...
  function upsertTicket(ticket) {
    const idx = DATA.tickets.findIndex((item) => item.ticketId === ticket.ticketId);
    if (idx >= 0) DATA.tickets[idx] = ticket;
    else DATA.tickets.unshift(ticket);
  }

  async function openTicketReal(ticketId) {
    try {
      const detail = await apiGet('/api/v1/tickets/' + encodeURIComponent(ticketId));
      const ticket = mapTicketDetailToTicket(detail);
      upsertTicket(ticket);
  ...
  async function classifyTicketReal(ticketId) {
    const form = document.getElementById('classification-form');
    if (!form) {
      showToast('操作失败', '缺少分类表单', 'error');
      return;
    }
    const values = new FormData(form);
    const payload = {
      managementUnitId: String(values.get('managementUnitId') || '').trim() || null,
      symptomId: String(values.get('symptomId') || '').trim() || null,
      reasonId: String(values.get('reasonId') || '').trim() || null,
      solutionMethodId: String(values.get('solutionMethodId') || '').trim() || null,
      customReason: String(values.get('customReason') || '').trim() || null,
      customSolution: String(values.get('customSolution') || '').trim() || null
    };
    try {
      await apiPatch('/api/v1/support/tickets/' + encodeURIComponent(ticketId) + '/classification', payload);
      await openTicketReal(ticketId);
  ...
  async function resolveTicketReal(ticketId) {
    const ticket = getTicketById(ticketId);
    const classification = ticket && ticket.classification ? ticket.classification : {};
    const solutionMethodId = classification.solutionMethodId || null;
    const customSolution = classification.customSolution || null;
    const resolution = customSolution || solutionMethodId || null;
    if (!resolution) {
      showToast('提交失败', '请先完成分类并填写解决方法', 'error');
      return;
    }
    try {
      await apiPost('/api/v1/support/tickets/' + encodeURIComponent(ticketId) + '/resolve', {
        resolution,
        resolutionType: null,
        solutionMethodId,
        customSolution
  ...
  function renderLoginScreen() {
    appRoot.innerHTML = `
      <div class="login-screen">
        <section class="login-card">
          <div class="login-brand"><span class="brand-mark">ITSM</span><div><strong>客服工单系统</strong><small>登录后处理工单</small></div></div>
  ...
  function currentUser() {
    return (ITSM_SESSION && ITSM_SESSION.user) ? ITSM_SESSION.user : currentProfile().me.data;
  }

  function currentTenant() {
    return (ITSM_SESSION && ITSM_SESSION.tenant) ? ITSM_SESSION.tenant : currentProfile().login.data.tenant;
  }

  function currentRoleCodes() {
    if (ITSM_SESSION && ITSM_SESSION.roles && ITSM_SESSION.roles.length) {
      return ITSM_SESSION.roles;
    }
  ...
  function renderUserHomePage() {
    const session = getCurrentSession();
    if (!session) {
      shell('<div class="empty-state"><div class="empty-icon">聊</div><strong>还没有会话</strong><p>点击「新建会话」开始咨询。</p></div>');
      return;
    }
  ...
  function renderSupervisorTicketDrawer(ticket) {
    ...
    `).join('');
    const requesterDisplayName = (ticket.requester && ticket.requester.displayName) || '—';
    const requesterDepartment = (ticket.requester && ticket.requester.departmentName) || '';
    const conversationSummary = (ticket.conversation && ticket.conversation.summary) || '暂无会话摘要';
    const conversationMessageCount = (ticket.conversation && ticket.conversation.messageCount != null) ? ticket.conversation.messageCount : 0;
    const resolutionSummary = (ticket.resolution && ticket.resolution.summary) || '暂无解决结果';
    const resolutionMethod = (ticket.resolution && ticket.resolution.method) || '暂无解决方式';
        ...
        <div class="drawer-body supervisor-drawer-body">
          <div class="detail-grid">
            <div class="detail-card">
              ...
              <div class="detail-row"><label>请求人</label><div>${escapeHtml(requesterDisplayName)} · ${escapeHtml(requesterDepartment)}</div></div>
            ...
            <div class="detail-card">
              ...
              <div class="detail-row"><label>消息数</label><div>${escapeHtml(String(conversationMessageCount))}</div></div>
          ...
          <div class="detail-card" style="margin-top:12px;">
            ...
            <div class="message-mini" style="margin-top:10px;">会话摘要：${escapeHtml(conversationSummary)}</div>
          </div>
          ...
          <div class="detail-card" style="margin-top:12px;">
            <h3>处理结果与评价</h3>
            <div class="detail-row"><label>结果摘要</label><div>${escapeHtml(resolutionSummary)}</div></div>
            <div class="detail-row"><label>解决方式</label><div>${escapeHtml(resolutionMethod)}</div></div>
      ...
      <aside class="drawer support-ticket-drawer">
        <div class="drawer-header support-ticket-drawer-header">
          <div class="drawer-header-main">
            ...
            <h2>${escapeHtml(ticket.title)}</h2>
            <p>${escapeHtml(requesterDisplayName)} · ${escapeHtml(requesterDepartment)}</p>
            <div class="meta-tags">
              <span class="status-tag ${statusClass(ticket.status)}">${escapeHtml(statusLabel(ticket.status))}</span>
              <span class="priority-tag ${priorityClass(ticket.priority)}">${escapeHtml(priorityLabel(ticket.priority))}优先级</span>
          ...
          <div class="support-ticket-layout">
            <div class="support-ticket-main">
              <div class="detail-card">
                ...
                <div class="detail-grid">
                  <div class="detail-row"><label>工单号</label><div>${escapeHtml(ticket.ticketNo)}</div></div>
                  <div class="detail-row"><label>提单人</label><div>${escapeHtml(requesterDisplayName)}</div></div>
              ...
              <div class="detail-card">
                ...
                <div class="message-mini" style="margin-top:10px;">会话摘要：${escapeHtml(conversationSummary)}</div>
                <div class="message-mini">消息数：${escapeHtml(String(conversationMessageCount))}</div>
              </div>
              <div class="detail-card">
                <h3>处理结果</h3>
                <div class="detail-row"><label>结果摘要</label><div>${escapeHtml(resolutionSummary)}</div></div>
        ...
        <div class="drawer-body">
          <div class="detail-grid">
            <div class="detail-card">
              ...
              <div class="detail-row"><label>会话摘要</label><div>${escapeHtml(conversationSummary)}</div></div>
              <div class="detail-row"><label>消息数</label><div>${escapeHtml(String(conversationMessageCount))}</div></div>
              <div class="detail-row"><label>更新时间</label><div>${escapeHtml(formatDateTime(ticket.updatedAt))}</div></div>
            </div>
            <div class="detail-card">
          ...
          ${isUser ? `
            <div class="detail-card" style="margin-top:12px;">
              <h3>处理结果</h3>
              <div class="message-mini">结果摘要：${escapeHtml(resolutionSummary)}</div>
              <div class="message-mini">解决方式：${escapeHtml(resolutionMethod)}</div>
          ...
          ` : `
            ...
            <div class="detail-card" style="margin-top:12px;">
              ...
              ${audit || '<div class="message-mini">暂无审计事件</div>'}
              <div class="divider"></div>
              <div class="message-mini">解决结果：${escapeHtml(resolutionSummary)}</div>
              <div class="message-mini">解决方式：${escapeHtml(resolutionMethod)}</div>
            </div>
          `}
        </div>
  ...
  function handleClick(event) {
    ...
    if (action === 'handoff') {
      ...
      handoffReal(session.sessionId);
      return;
    }

    if (action === 'create-ticket') {
      createTicketFromSessionReal();
      return;
    }
    ...
    if (action === 'open-ticket') {
      openTicketReal(target.dataset.ticketId);
      return;
    }
    ...
    if (action === 'save-classification') {
      ...
      classifyTicketReal(target.dataset.ticketId);
      return;
    }

    if (action === 'resolve-ticket') {
      if (!hasPermission('ticket:resolve')) {
        showToast('无权操作', 'ROLE_FORBIDDEN', 'error');
        return;
      }
      resolveTicketReal(target.dataset.ticketId);
      return;
    }
    ...
    if (action === 'refresh-config') {
      loadRealDictTab(state.activeDictTab).finally(() => {
        renderStart();
        renderOverlayWrapper();
      });
      return;
  ...
  function handleSubmit(event) {
    ...
    if (form.id === 'login-form') {
      ...
      doRealLogin(tenantId, account, password).catch((error) => showToast('登录失败', error.message, 'error'));
      return;
    }
    ...
    if (form.id === 'dict-form') {
      ...
      if (overlay.mode === 'edit') updateDictItemReal(overlay.itemId);
      else createDictItemReal();
      return;
    }
    ...
    if (form.id === 'disable-form') {
      ...
      disableDictItemReal(overlay.itemId);
      return;
    }
  }
  ...
  function renderInitial() {
    startSlaClock();
    if (!ITSM_SESSION || !ITSM_SESSION.accessToken) {
      renderLoginScreen();
      return;
    }
    if (redirectToPortalIfStandalone()) return;
